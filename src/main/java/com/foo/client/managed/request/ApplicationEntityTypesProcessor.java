...
public abstract class ApplicationEntityTypesProcessor<T> {
    ...
    public static Set<java.lang.Class<? extends com.google.gwt.requestfactory.shared.EntityProxy>> getAll() {
        ...
        rtn.add(TimerProxy.class);
        return Collections.unmodifiableSet(rtn);
    }

    private static void process(com.foo.client.managed.request.ApplicationEntityTypesProcessor<?> processor, Class<?> clazz) {
        if (TimerProxy.class.equals(clazz)) {
            processor.handleTimer((TimerProxy) null);
            return;
        }
        processor.handleNonProxy(null);
    }

    private static void process(com.foo.client.managed.request.ApplicationEntityTypesProcessor<?> processor, Object proxy) {
        if (proxy instanceof TimerProxy) {
            processor.handleTimer((TimerProxy) proxy);
            return;
        }
    ...
    public abstract void handleTimer(TimerProxy proxy);
